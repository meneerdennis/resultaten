...
export default function LottoCharts() {
  const [cumData, setCumData] = useState([]);
  const [monthData, setMonthData] = useState([]);
  const [monthTicks, setMonthTicks] = useState([]); // enkel 1 tick per maand
  const [year, setYear] = useState("");
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    (async () => {
      // 1) alle draws ophalen en sorteren (ASC) op id = YYYY-MM-DD
      ...
      // 3) cumulatieve winst + maandticks bepalen
      let running = 0;
      const cum = [];
      const ticks = [];
      let lastMonth = "";
      ...
      for (const r of inYear) {
        ...
        const monthKey = r.id.slice(0, 7); // "YYYY-MM"
        if (monthKey !== lastMonth) {
          ticks.push(r.id); // eerste dag van de maand als tick
          lastMonth = monthKey;
        }
  ...
  return (
    <div>
      <h2 style={{ marginBottom: 8 }}>Lotto – Grafieken ({year})</h2>
        ...
        <ResponsiveContainer>
          <LineChart data={cumData}>
            ...
            <XAxis
              ...
              ticks={monthTicks}
              tickFormatter={(d) => {
                const dt = new Date(d); // "YYYY-MM-DD"
        ...
        <ResponsiveContainer>
          <BarChart data={monthData}>
            ...
            <XAxis
              dataKey="month"
              tickFormatter={(m) => {
                const d = new Date(m + "-01"); // "YYYY-MM-01"
